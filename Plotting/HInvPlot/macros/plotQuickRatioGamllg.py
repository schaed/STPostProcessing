...
def Draw(hname1,f1les=[],can=None,GetError=True, hpath1all=[''],hpath2all=[''],hpath3all=[],extra=''):
    ...
    if h3:
        AddOverflow(h3)
    if hname.count('tmva'):
        h1.GetXaxis().SetRangeUser(0,1.0)
    ...
    if hname1=='jj_deta':
        ...
        h2.Rebin(rebin)
        if h3: h3.Rebin(rebin)        
    if hname1=='met_tst_et' or hname1=='met_tst_nolep_ph_dphi':
        h1.Rebin(rebin)
        h2.Rebin(rebin)
        if h3: h3.Rebin(rebin)
    if hname1=='jj_dphi':
    ...
    leg.SetBorderSize(0)
    leg.SetFillColor(0)
    myproc='#gamma+Z#rightarrow ll'
    if hpath1.count('gamwcr_'):
        myproc='#gamma+W#rightarrow l#nu'
    leg.AddEntry(h1,'Sherpa 2.2.8 '+myproc)
    leg.AddEntry(h2,'Sherpa 2.2.2 '+myproc) 
    if h3:
        leg.AddEntry(h3,'MG FxFx '+myproc)            
    leg.Draw()
    ...
    if hname=='ph_pt_lead' or hname1=="phPt":
        ...
        if not GetError:
            ...
            pad1.SetLogx(1)
            pad2.SetLogx(1)
    elif hname.count('tmva'):
        hratio.GetXaxis().SetTitle('Keras Score')
        hratio.GetXaxis().SetRangeUser(0,1.0)
    elif hname.count('mtgammet'):
        hratio.GetXaxis().SetTitle('m_{T}(#gamma,MET) [GeV]')
    elif hname.count('met_tst_nolep_ph_dphi'):
        hratio.GetXaxis().SetTitle('#Delta#phi(MET,#gamma)')        
    elif hname=='elpos_pt':
        hratio.GetXaxis().SetTitle('Electron p_{T} [GeV]')
        if not GetError:
            pad1.SetLogy(1)
    ...
    if hratio2:
        hratio2.Draw('same')
    midpoint = hratio.Clone()
    if hratio2:
        midpoint = hratio2.Clone()    
    midpoint.SetFillColor(ROOT.kBlue)
    midpoint.SetLineColor(ROOT.kBlue)
    midpoint.SetMarkerColor(ROOT.kBlue)
    midpoint.SetLineStyle(2)
    midpoint.SetMarkerStyle(ROOT.kFullTriangleUp)            
    midpoint.SetFillStyle(3001)
    midpoint.SetLineWidth(1)
    for i in range(0,midpoint.GetNbinsX()+1):
        m=midpoint.GetBinContent(i)
        m=(m-1.0)/2.0+1.0
        merr = abs(m-1.0)
        midpoint.SetBinContent(i,m)
        midpoint.SetBinError(i,merr)
    midpoint.Draw('same E3')
    midpoint.Draw('LF2 SAME ')
    legR = ROOT.TLegend(0.7,0.8,0.92,0.95)
    legR.SetBorderSize(0)
    legR.SetFillColor(0)
    legR.AddEntry(midpoint,'Midpoint with error')
    legR.Draw()
    can.Update()
    if options.wait:
        can.WaitPrimitive()
...
def Fit(_suffix=''):
    ...
    Style();
    #f1 = ROOT.TFile.Open('/tmp/fxfxADbvCuts.root')
    #f1up = ROOT.TFile.Open('/tmp/fxfxADbvCuts.root')
    #f1dw = ROOT.TFile.Open('/tmp/fxfxADbvCuts.root')
    #f1 = ROOT.TFile.Open('/tmp/fxfxADcutBVeto.root')
    #f1up = ROOT.TFile.Open('/tmp/fxfxADcutBVeto.root')
    #f1dw = ROOT.TFile.Open('/tmp/fxfxADcutBVeto.root')    
    f1 = ROOT.TFile.Open('/tmp/fxfxADEcutBVeto.root')
    f1up = ROOT.TFile.Open('/tmp/fxfxADEcutBVeto.root')
    f1dw = ROOT.TFile.Open('/tmp/fxfxADEcutBVeto.root')
    #f1 = ROOT.TFile.Open('/tmp/fxfxEcutBVeto.root')
    #f1up = ROOT.TFile.Open('/tmp/fxfxEcutBVeto.root')
    #f1dw = ROOT.TFile.Open('/tmp/fxfxEcutBVeto.root')
    #f1 = ROOT.TFile.Open('/tmp/fxfxADEcutSR.root')
    #f1up = ROOT.TFile.Open('/tmp/fxfxADEcutSR.root')
    #f1dw = ROOT.TFile.Open('/tmp/fxfxADEcutSR.root')

    h1_norm=1.0 #36.100
    h2_norm=1.0 #36.100 #hggf, hvh, hvbf
    hnames=['met_tst_et','jj_mass','jj_deta','jj_dphi','phPt','n_jet','j3Pt','jj_deta','met_tst_et','met_tst_nolep_et','tmva_variableBin','tmva','jj_mass_variableBinGam','met_tst_nolep_ph_dphi','mtgammet','mtgammet_variableBinGam']
    hnames=['jj_mass_variableBinGam','met_tst_nolep_ph_dphi','mtgammet_variableBinGam']    #'truth_jj_mass_variableBin',
    #hnames = options.var.split(',')

    path1=['pass_gamzcr_allmjj_ll_Nominal/plotEvent_zgam/']
    path2=['pass_gamzcr_allmjj_ll_Nominal/plotEvent_zgamewk/']
    path3 = ['pass_gamzcr_allmjj_ll_Nominal/plotEvent_tall/']
    for hname in hnames:
        Draw(hname,[f1,f1up,f1dw],can,GetError=False, hpath1all=path1,hpath2all=path2,hpath3all=path3,extra='_SRllg')

    path1=['pass_gamwcr_allmjj_l_Nominal/plotEvent_wgam/']
    path2=['pass_gamwcr_allmjj_l_Nominal/plotEvent_wgamewk/']
    path3 = []#'pass_gamwcr_allmjj_l_Nominal/plotEvent_tall/']
    for hname in hnames:
        Draw(hname,[f1,f1up,f1dw],can,GetError=False, hpath1all=path1,hpath2all=path2,hpath3all=path3,extra='_SRllg')

    # SR - looseCR
    path1=['pass_gamzcr_loosecr_ll_Nominal/plotEvent_zgam/']
    path2=['pass_gamzcr_loosecr_ll_Nominal/plotEvent_zgamewk/']
    path3 = ['pass_gamzcr_loosecr_ll_Nominal/plotEvent_tall/']
    for hname in hnames:
        Draw(hname,[f1,f1up,f1dw],can,GetError=False, hpath1all=path1,hpath2all=path2,hpath3all=path3,extra='_LooseCRllg')
    # SR - nodphi
    path1=['pass_gamzcr_nodphi_ll_Nominal/plotEvent_zgam/']
    path2=['pass_gamzcr_nodphi_ll_Nominal/plotEvent_zgamewk/']
    path3 = ['pass_gamzcr_nodphi_ll_Nominal/plotEvent_tall/']
    for hname in hnames:
        Draw(hname,[f1,f1up,f1dw],can,GetError=False, hpath1all=path1,hpath2all=path2,hpath3all=path3,extra='_NoDPhillg')        
    # SR - rev ph cen
    path1=['pass_gamzcr_loosecrrevphcen_ll_Nominal/plotEvent_zgam/']
    path2=['pass_gamzcr_loosecrrevphcen_ll_Nominal/plotEvent_zgamewk/']
    path3 = ['pass_gamzcr_loosecrrevphcen_ll_Nominal/plotEvent_tall/']
    for hname in hnames:
        Draw(hname,[f1,f1up,f1dw],can,GetError=False, hpath1all=path1,hpath2all=path2,hpath3all=path3,extra='_loosecrrevphcen')
    # SR - rev jet cen
    path1=['pass_gamzcr_loosecrrevjetcen_ll_Nominal/plotEvent_zgam/']
    path2=['pass_gamzcr_loosecrrevjetcen_ll_Nominal/plotEvent_zgamewk/']
    path3 = ['pass_gamzcr_loosecrrevjetcen_ll_Nominal/plotEvent_tall/']
    for hname in hnames:
        Draw(hname,[f1,f1up,f1dw],can,GetError=False, hpath1all=path1,hpath2all=path2,hpath3all=path3,extra='_loosecrrevjetcen')                
