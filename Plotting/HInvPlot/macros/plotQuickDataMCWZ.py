...
p.add_option('--var',           type='string', default='jj_mass,n_jet,met_cst_jet,jj_mass_variableBin,jj_deta,jj_dphi,met_tst_nolep_et,mll', dest='var') #SherpaVTruthPt
p.add_option('--filename','-f', type='string', default='/tmp/jetVetoALL120.root', dest='filename')
...
p.add_option('--doJetVeto',          action='store_true', default=False,   dest='doJetVeto')
...
def GetHist(hname1, hpath, f1,  can, sigsamples, doData=False):
    ...
    for hsamp in sigsamples:
        ...
        print hname
        h1b = f1.Get(hname)
        if not h1b and sigsamples[0].count('Mad'):
            return None
...
def Draw(can, hname, bkgsub1hist, bkgsub2hist,  sig1hist, sig2hist, hpath1, hpath2, mghist=[], dataMinBkg=True, doNormalized=True):
    ...
    if doNormalized:
        ...
        if dataMinBkg:
            h1.GetYaxis().SetTitle('V strong Jet Veto Eff. (Data - Bkg)')
        else:
            h1.GetYaxis().SetTitle('V strong Jet Veto Eff.')
    ...
    if not doNormalized:
        leg.AddEntry(h3,'MC '+type_sample+num_name)
        leg.AddEntry(h4,'MC '+type_sample+den_name)
    leg.Draw()
    ...
    hratio = h1.Clone()
    intden = h1.Integral()
    if intden>0.0 and doNormalized:
    ...
    intden = h3.Integral()
    if intden>0.0 and doNormalized:
        hratioMC.Scale(h4.Integral()/intden)    
    hratioMC.Divide(h4)
    hratioMC.SetLineColor(2)
    hratioMC.SetMarkerColor(2)
    ...
    if h5:
        ...
        intden = h5.Integral()
        if intden>0.0 and doNormalized:
            hratioMCMG.Scale(h6.Integral()/intden)    
        hratioMCMG.Divide(h6)
        hratioMCMG.GetYaxis().SetRangeUser(0,2.0)
        hratioMCMG.SetLineColor(3)
        hratioMCMG.SetMarkerColor(3)
    hratio.GetYaxis().SetRangeUser(0,2.0)
    hratioMC.GetYaxis().SetRangeUser(0,2.0)
    if hname1 in ['jj_mass']:
        pad1.SetLogy(1)
    else:
        pad1.SetLogy(0)
    pad2.SetLogy(0)
    ...
    leg1.AddEntry(hratioMC,'Sherpa MC ratio')    
    if hratioMCMG:
        hratioMCMG.Draw('same')
        leg1.AddEntry(hratioMCMG,'MG MC ratio')
    leg1.Draw()
    ...
    #else:
    can.SaveAs(hname1+'_'+comp1+'_'+comp2+'_'+options.region+type_sample_out+extra_text_save+'.pdf')
...
def Fit(_suffix=''):
    ...
    #f2 = ROOT.TFile.Open('/tmp/v45ALLGamv15noJV.root')    
    ...
    reg= options.region
    doJetVeto=options.doJetVeto
    sigsamples4 = ['zqcdMad','zewk']
    sigsamples3 = ['wqcdMad','wewk']
    sigsamples2 = ['wqcd']
    bkgsamples2 = ['data','zewk','tall','zqcd','wewk']#,'dqcd']
    sigsamples1 = ['zqcd']
    bkgsamples1 = ['data','wewk','wqcd','tall','zewk']#,'dqcd']
    path1='pass_zcr_'+reg+'_ll_Nominal/plotEvent_'
    path2='pass_wcr_'+reg+'_l_Nominal/plotEvent_'
    for hname in hnames:
        ...
        # Loading Madgraph
        sig3hist = GetHist(hname, path2, f1, can, sigsamples3, doData=False)
        sig4hist = GetHist(hname, path1, f1, can, sigsamples4, doData=False)
        mghist=[]
        if sig3hist and sig4hist:
            mghist=[sig4hist,sig3hist]

        if doJetVeto:
            reg='allmjjalljetnodphi'
            path1='pass_zcr_'+reg+'_ll_Nominal/plotEvent_'
            path2='pass_wcr_'+reg+'_l_Nominal/plotEvent_'
            alljetsig1hist = GetHist(hname, path1, f1, can, sigsamples1, doData=False)
            alljetbkgsub1hist = GetHist(hname, path1, f1, can, bkgsamples1, doData=True)
            alljetsig2hist = GetHist(hname, path2, f1, can, sigsamples2, doData=False)
            alljetbkgsub2hist = GetHist(hname, path2, f1, can, bkgsamples2, doData=True)
            ...
            if False:
                sig1hist.Divide(alljetsig1hist)
                bkgsub1hist.Divide(alljetbkgsub1hist)
                sig2hist.Divide(alljetsig2hist)
                bkgsub2hist.Divide(alljetbkgsub2hist)
            else:
                sig1hist.Divide(sig1hist,alljetsig1hist,1,1,"B")
                bkgsub1hist.Divide(bkgsub1hist,alljetbkgsub1hist,1,1,"B")
                sig2hist.Divide(sig2hist,alljetsig2hist,1,1,"B")
                bkgsub2hist.Divide(bkgsub2hist,alljetbkgsub2hist,1,1,"B")
            Draw(can, hname, bkgsub1hist,bkgsub2hist, sig1hist, sig2hist, path1, path2, mghist=mghist, dataMinBkg=True,doNormalized=False)            
        else:
            Draw(can, hname, bkgsub1hist,bkgsub2hist, sig1hist, sig2hist, path1, path2, mghist=mghist, dataMinBkg=True,doNormalized=True)
    sys.exit(0)
