...
p.add_option('--var',           type='string', default='jj_mass_variableBin', dest='var') #SherpaVTruthPt jj_mass,n_jet,met_cst_jet,jj_mass_variableBin,jj_deta,jj_dphi,met_tst_nolep_et,mll
p.add_option('--filename','-f', type='string', default='/tmp/ktPDFVar/out_Nominal', dest='filename')
...
def DrawPDF(can, hname, histosIn, hpath1, sampl, histosForRatio=[]):

    histos=[]
    iw=0
    for h in histosIn:
        hnew = h.Clone()
        if len(histosForRatio)>0:
            hnew.Divide(histosForRatio[iw])
        histos+=[hnew]
        iw+=1
    
    twoD = ROOT.TH2F('h2'+sampl[0],'h2'+sampl[0],histos[0].GetNbinsX(),0.0,histos[0].GetNbinsX()+1,100,0.5,1.5)
    can.Clear()
    hname1=hname
    ncolor=1
    for h in histos:
    ...
    extra_text='VBF H125'
    rebin=2
    if sampl.count('_hggf'):
        extra_text='ggF H125'
        extra_text_save='_ggf'
        rebin=5
    elif sampl.count('_hvh'):
        extra_text='VH H125'
        extra_text_save='_hvh'
        rebin=5
    elif sampl[0].count('_zqcd'):
        extra_text='Z Strong'
    elif sampl[0].count('_wqcd'):
        extra_text='W Strong'
    if len(histosForRatio):
        extra_text='Z/W Strong'
    ...
    hratios=[]
    hratio = histos[0].Clone()
    hratio.Divide(hratio)
    for i in range(1,len(histos)):
        hratiob = histos[i].Clone()
        hratiob.Divide(histos[0])
        hratios+=[hratiob]
        for ib in range(1,hratiob.GetNbinsX()+1):
            twoD.Fill(ib, hratiob.GetBinContent(ib))
    ...
    hratio.GetYaxis().SetTitle('PDF Var/Nom')
    ...
    #ks_text2.Draw()
    if len(histosForRatio)>0:
        extra_text_save+='_WtoZ'
    can.Update()
    if options.wait:
        can.WaitPrimitive()
    can.SaveAs(hname1+'_'+comp1+'_'+options.region+type_sample_out+extra_text_save+'.pdf')

    can.Clear()
    twoD.GetYaxis().SetTitle('PDF Var / Nom')
    twoD.GetXaxis().SetTitle('Kinematic Bins')    
    twoD.Draw('colz')
    can.Update()
    if options.wait:
        can.WaitPrimitive()
    can.SaveAs(hname1+'_'+comp1+'_'+options.region+type_sample_out+extra_text_save+'2d.pdf')
    #can.Clear()
    #twoD.FitSlicesY()
    #twoD.GetXaxis().SetTitle('Kinematic Bins')    
    #twoD.Draw('colz')
...
def Fit(_suffix=''):
    ...
    Style();
    f1s=[ ROOT.TFile.Open(options.filename+'0.root')]
    #for i in range(11,110):
    for i in range(115,147):
        f1 = ROOT.TFile.Open(options.filename+'%s.root' %i)
        f1s+=[f1]
    ...
    path1='pass_sr_'+reg+'_nn_Nominal/plotEvent_'
    path2='pass_wcr_'+reg+'_l_Nominal/plotEvent_'
    for hname in hnames:
        z1hists=[]
        w1hists=[]
        for f1a in f1s:
            z1hist = GetHist(hname, path1, f1a, can, sigsamples1, doData=False)
            print z1hist.Integral()
            w1hist = GetHist(hname, path2, f1a, can, sigsamples2, doData=False)
            z1hists+=[z1hist]
            w1hists+=[w1hist]

        DrawPDF(can, hname, z1hists, path1, sigsamples1)
        DrawPDF(can, hname, w1hists, path2, sigsamples2)
        DrawPDF(can, hname, z1hists, path1, sigsamples1, histosForRatio=w1hists)
    sys.exit(0)
