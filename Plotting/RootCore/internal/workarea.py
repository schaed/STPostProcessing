...
class WorkArea(object) :
    ...
    def ut_test_list(self):
        """returns: the list of tests we have"""
        result = []
        name_expr = re.compile("^(ut_[^.]*)(.*)$")
        for pkg in self.packages():
            testdir = os.path.join(pkg.srcdir(), "test")
            if os.path.isdir(testdir):
                for file in os.listdir(testdir):
                    match = name_expr.match(file)
                    if match:
                        if match.group(2) == ".cxx":
                            result.append(os.path.join(pkg.testdir(), match.group(1)))
                        elif match.group(2) == ".sh" or match.group(2) == ".py":
                            result.append(os.path.join(testdir, file))
        return result
