...
def rc_compile (argv):
    ...
    if not args.noLoadPackages:
        ...
        if (shell_exec(["rc", "root", "-l", "-b", "-q",
                        wa.dir() + "/scripts/load_packages.C"],
                       returnRC=True) != 0 or
...
def rc_grid_submit (argv):
    ...
    for pkg in wa.packages():
        ...
        if pkg.srcdir() == mypkg.srcdir():
            print "using " + pkg.name() + " source from release"
            pass
        else:
            print "copying " + pkg.name() + " source"
            ignore = [".svn", "*~"]
            if pkg.name() == "RootCore":
                ignore += ["bin", "include", "obj", "rootcore_config",
                           "root_config_*", "load_packages_info_*",
                           "load_packages_success", "data", "lib", "python",
                           "user_scripts"]
                pass
            rel_dir = wa.relPath (DIR, pkg.srcdir(), pkg.srcdir())
            if rel_dir[0] != "/" and rel_dir != ".":
                ignore.append (os.path.basename (rel_dir))
                pass
            if rel_dir != ".":
                shutil.copytree (pkg.srcdir(), mypkg.srcdir(), symlinks = True,
                                 ignore = shutil.ignore_patterns (*ignore))
                if pkg.name() == "RootCore":
                    shell_exec (["chmod", "-R", "u+w", mypkg.srcdir()])
                    pass
                pass
            #wa_new.packages().addPkg (mypkg)
            pass
    ...
    if args.noBuild:
        for config in os.listdir (wa.obj()):
            ...
            for pkg in wa.packages():
                mypkg = wa_new.packages().getPkg (pkg.name())
                if pkg.outdir() == mypkg.outdir():
                    print "using " + pkg.name() + " object files from release"
                    pass
                else:
                    print "copying " + pkg.name() + " for " + config
                    for dir in "bin", "lib", "test-bin":
                        source = os.path.join (pkg.outdir(), config,
                                               pkg.name(), dir)
                        target = os.path.join (mypkg.outdir(), config,
                                               pkg.name(), dir)
                        if os.path.isdir (source):
                            ...
                            pass
                        pass
                    mypkg.linkPreconfigure()
                    mypkg.linkPrecompile()
                    mypkg.linkPostcompile()
                    pass
                pass
            pass
        pass
...
def rc_test_ut (argv):
    ...
    for test in tests:
        ...
        try:
            ...
            mkdir_p (dir)
            # Execute the test
            shell_exec (["time", test], workDir=dir, noReturn=True,
                        newEnv=newEnv)
...
def rc_clean (argv):
    ...
    try:
        ...
        for dir in p.join (wa.bin(), "lib"), p.join (wa.bin(), "bin"), wa.obj():
            path = p.join (dir, wa.arch())
            if p.isdir (path):
                shutil.rmtree (path)
                pass
            pass
        path = p.join (wa.bin(), "root_config_" + wa.arch())
        if p.isfile (path):
            os.remove (path)
            pass
    finally:
            ...
            if location[0] != "/":
                ...
                if md5sum:
                    ...
                    if hash.hexdigest() != md5sum:
                        s = "  checksum error: md5sum=%g reference=%g"
...
def rc_external_download (argv):
    ...
    if not args.no_flags:
        shell_init.setEnv ("CC", wa.getCxx())
        ...
        shell_init.setEnv ("CFLAGS", package.m_rcpkg.getCxxFlags (external=True))
...
def rc_tag_package (argv):
    ...
    if args.tag :
        ...
        # NOTE: the hardcoded indices in the following sections
    ...
    if args.batch:
        ...
        answer = ask_question ("please confirm creating tag " + tag,
                               ["yes", "no"])
        if answer == "yes":
            make_tag = True
            pass
...
def rc_compile_pkg (argv):
    ...
    pkg.linkPreconfigure ()
    pkg.compile (ignoreErrors = args.cont)
    pass
