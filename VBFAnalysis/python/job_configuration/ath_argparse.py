...
class AthenaArgumentParser(argparse.ArgumentParser):
  """
    Parse commandline arguments in a job option.
    Extends argparse.ArgumentParser
  """

  def __init__(self, joboption_name=None, stack_depth=1, print_caller_help=False, prog=None, **kwargs):
    """
      Create the parser
    ...
    if print_caller_help:
      ...
      self.caller_help = ""
    super(AthenaArgumentParser, self).__init__(prog=prog, **kwargs)
    super(AthenaArgumentParser, self).add_argument("remaining_scripts", nargs="*", help="Any remaining scripts to be run by athena after this job option. Mainly used to run the 'post' script on the grid.")
  ...
  def parse_args(self, args=None, namespace=None):
    ...
    if args is None:
      args = opts.user_opts
    return super(AthenaArgumentParser, self).parse_args(args, namespace)

  def parse_known_args(self, args=None, namespace=None):
    """Override base class to use the leftover athena arguments by default"""
    if args is None:
      args = opts.user_opts
    return super(AthenaArgumentParser, self).parse_known_args(args, namespace)

  def format_help(self):
    text = super(AthenaArgumentParser, self).format_help()
    return text + "\n" + self.caller_help
